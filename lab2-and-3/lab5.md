...
```scheme(remove-trailing l n)``` премахва всички срещания на числото ```n```, започвайки отзад на списъка.
...
```scheme(remove-leading l n)``` e като ```remove-trailing```, само че започва маханeто на даденото число ```n``` от началото на списъка.
...
```scheme(count-n l n)``` връща срещанията на числото ```n``` в списъка ```l```

## map-argument

```scheme(map-argument list-of-functions x)``` приема списък от едноаргументи функции и цяло число ```x```, като връща нов списък, съставен от резултатът на всички функции, извикани с аргумент ```x```

## prefix?

```scheme(prefix? l1 l2)``` връща истина, ако списъкът ```l1``` е префикс на списъка ```l2```

## do-while

```scheme(do-while l f pred?)``` взима списък и връща нов списък, който се строи по следния начин:
